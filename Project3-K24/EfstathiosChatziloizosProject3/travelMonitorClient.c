...
int main(int argc, char** argv)
{
	...
	// Boh8htikes metablhtes gia ta sockets
    ...
    for (int i = 0; i < num_Monitors; i++)
    {
		...
		// Eisagwgh twn args twn monitorServer
		...
		// Pros8hkh twn paths sta subdirectories
    ...
    while(!flag)
	{
		...
		else if (strcmp(command_name,"travelRequest")==0)
		{
			...
			else
			{
				...
				if(i!=0)
				{
					...
					// Apostolh travelRequest command sto monitorServer poy diaxeirizetai to countryFrom
					...
					else if(strcmp(reply,"YES")==0)
					{
						...
						// Lhpsh hmeromhnias
						if(read(newsock[countryFromIndex],&readBytes, sizeof(int)) < 0)     {perror("read");    return -1;}
						if(read(newsock[countryFromIndex],reply, readBytes) < 0)     {perror("read");    return -1;}
		...
		else if(strcmp(command_name,"searchVaccinationStatus")==0)
		{
			...
			for(i=0; i < num_Monitors; i++)
			{
				// Apostolh searchVaccinationStatus command sta monitorServer
	...
	// Elef8erwsh mnhmhs/sockets kai desmeymenwn domwn
	for(int i=0; i < counter; i++)
	{
		free(countries[i]);
	}
